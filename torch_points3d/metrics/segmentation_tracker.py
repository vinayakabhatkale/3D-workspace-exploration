...
class SegmentationTracker(BaseTracker):
    def __init__(
        self, dataset, stage="train", wandb_log=False, use_tensorboard: bool = False, ignore_label: int = IGNORE_LABEL, dataset_opt: dict=None
    ):
        ...
        """
        super(SegmentationTracker, self).__init__(stage, wandb_log, use_tensorboard, dataset_opt=dataset_opt)
    ...
    def _compute_metrics(self, outputs, labels):
        ...
        acc_per_class = self.confusion_matrix.diagonal()/self.confusion_matrix.sum(axis=1) 
        self._acc_per_class = {
            i: "{:.2f}".format(100 * v)
    ...
    def get_metrics(self, verbose=False, final: bool=False) -> Dict[str, Any]:
        ...
        """
        if final:
            return self.cross_val_final_metrics
        ...
        if verbose:
            ...
            metrics["{}_macc_per_class".format(self._stage)] = self._acc_per_class
