...
class Trainer:
    ...
    def _initialize_trainer(self):
        ...
        if self.cross_val is True and not hasattr(self._cfg.data, "fold"):
            self._cfg.data.fold = 0
        ...
        # Checkpoint
        check_name = self._cfg.model_name if self.cross_val is False else f"{self._cfg.model_name}{self._cfg.data.fold}"
        self._checkpoint: ModelCheckpoint = ModelCheckpoint(
            self._cfg.training.checkpoint_dir, check_name,
    ...
    def reset_trainer(self):
        ...
        del self._device
        del self._model
        self.history = self._tracker.history

    def train(self):
        self._is_training = True

        for i in range(self.kfold):
            if self.cross_val is True and self._cfg.data.fold > 0:
                self.reset_trainer()
                self._initialize_trainer()
                self._tracker.history = self.history
        ...
        if self.cross_val:
            self._tracker.print_summary(final=True)
                ...
                with Ctq(loader) as tq_loader:
                    for data in tq_loader:
                        ...
                        if self.profiling:
                            if i > self.num_batches:
                                return 0
            #Stefanie commented the below code since cross_val is not used atm.
            # if cross validation is used for each fold the metrics should be stored
            if self._cfg.cross_val.use:
                if epoch == self._cfg.training.epochs -1:
                    self._tracker.store_metrics_for_kfold()
                    self._cfg.data.fold = self._cfg.data.fold + 1

                if self._cfg.data.fold == self._cfg.cross_val.kfold:
                    self._tracker.finalize_cross_val_metrics()
    ...
    @property
    def cross_val(self):
        if hasattr(self._cfg, "cross_val"):
            return getattr(self._cfg.cross_val, 'use')
        else:
            return False
    ...
    def kfold(self):
        if hasattr(self._cfg, "cross_val"):
            return getattr(self._cfg.cross_val, 'kfold')
        else:
            return 1
